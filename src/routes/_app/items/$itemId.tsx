...
function ItemPage() {
  ...
  const item = useQuery(getItemOptions({ path: { itemId }, query: { userId } }))

  return (
    <div {...stylex.props(styles.page)}>
      <h1 {...stylex.props(styles.title)}>{item.data?.Name ?? ' '}</h1>
      <p {...stylex.props(styles.muted)}>{item.data?.Type} · Detail page coming next.</p>
    </div>
  )
}
